...
if __name__=="__main__":
    ...
    #2. convertInteger(y_dataset) -  라벨이 존재하는 경우 y_dataset 라벨데이터를 정수형으로 변환한다,
    #      ret - integer List type
    #3. Label_One_hot(y_dataset) - 라벨이 존재하는 경우 라벨리스트를 받아서 원핫인코딩 변환한다
    #4. int_one_hot(seLf,y_intdataset,LabeLdata=None)
    #                            - 정수형 데이터를 받아서 원핫인코딩을 리턴한다.
    #5. convert_LabeL(seLf,oharr,LabeL_List=None)
    #                            - 원핫인코딩된 데이터 및 라벨테스트
