...
plt.plot(fhist.history["val_loss"],label="val_acc")
plt.legend()
plt.show()
...
#select_6-0.04.keras
optimal_model = tf.keras.models.load_model(f"{SPATH}select_6-0.04.keras.keras");
res_opti = optimal_model.evaluate(x_test,y_test)
res = model.evaluata(x_test,y_test)
...
y_pred = optimal_model.predict(x_tset)
plt.figurt(figsize=(8,3))
for i,d in enumerate(rarr):
    plt.subplot(2,5,i+1)
    plt.imshow(x_tset[i],cmap="gray")
